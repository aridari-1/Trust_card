...
function NavButtons({
  onBack, onNext, nextLabel = "Continue →", canNext = true, loading = false,
}: {
...
function Shell({ step, total = 4, children }: { step: number; total?: number; children: React.ReactNode }) {
  return (
    <main className="min-h-screen bg-[#080808] text-white flex items-center justify-center px-4 py-12">
      ...
      <div className="relative z-10 w-full max-w-lg">
        <div className="flex items-center gap-2 mb-8">
          ...
          <span className="text-xs font-semibold tracking-widest uppercase text-white/50">TrustCard</span>
        </div>
...
function OnboardingInner() {
  ...
  async function checkUsername() {
    ...
    if (data) { setUsernameError("This username is already taken."); setUsernameAvailable(false); return false; }
  ...
  async function createProfile() {
    ...
    if (types && types.length > 0) setVouchTypeId(types[0].id);
    setSubmitting(false);
    setStep(3);
  }
  ...
  async function sendFirstVouch() {
    ...
    setVouchLink(`${location.origin}/vouch/${data.public_token}`);
  }
  ...
  if (step === 0) return (
    <Shell step={0}>
      ...
      <p className="text-sm text-white/40 mb-6">This is how you'll appear on your TrustCard.</p>
      <input className={inputCls} placeholder="Full name" value={fullName}
        onChange={(e) => setFullName(e.target.value)}
        onKeyDown={(e) => e.key === "Enter" && fullName.trim() && setStep(1)} autoFocus />
  ...
  if (step === 1) return (
    <Shell step={1}>
      ...
      <div className="relative">
        <input
          className={`${inputCls} pr-10 ${usernameError ? "border-red-500/60" : usernameAvailable ? "border-green-500/60" : ""}`}
      ...
      <p className="text-[11px] text-white/20 mt-1.5">Only lowercase letters, numbers, and underscores.</p>
      {usernameError && <p className="text-sm text-red-400 mt-2">{usernameError}</p>}
      <NavButtons onBack={() => setStep(0)} onNext={advanceFromStep1}
        nextLabel={usernameChecking ? "Checking…" : "Continue →"}
        canNext={username.length >= 3 && !usernameChecking} loading={usernameChecking} />
    </Shell>
  );
  ...
  if (step === 2) return (
    <Shell step={2}>
      ...
      <p className="text-sm text-white/40 mb-6">All optional — you can fill this in later.</p>
      <div className="space-y-3">
        <input className={inputCls} placeholder="School or university" value={school} onChange={(e) => setSchool(e.target.value)} />
        <input className={inputCls} placeholder="Major or field" value={major} onChange={(e) => setMajor(e.target.value)} />
        <textarea className={`${inputCls} min-h-24 resize-none`}
          placeholder="Short bio — what do you want people to know about you?"
          value={bio} onChange={(e) => setBio(e.target.value)} maxLength={200} />
        {bio.length > 0 && <p className="text-[11px] text-white/20 text-right">{bio.length} / 200</p>}
      </div>
      {submitError && <p className="text-sm text-red-400 mt-4">{submitError}</p>}
      <NavButtons onBack={() => setStep(1)} onNext={createProfile}
        nextLabel={submitting ? "Creating profile…" : "Create my TrustCard →"} loading={submitting} />
    </Shell>
  );
    ...
    if (vouchLink) return (
      <Shell step={3}>
        ...
        <div className="grid grid-cols-2 gap-2 mb-6">
          <a href={`https://wa.me/?text=${encodeURIComponent(`Hey! I just set up my TrustCard — would you vouch for me? Takes 2 min: ${vouchLink}`)}`}
            target="_blank" rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 rounded-xl border border-white/10
                       bg-white/5 px-4 py-3 text-sm text-white/60 hover:text-white hover:border-white/20 transition-colors">
            <span>💬</span> WhatsApp
          </a>
          <a href={`mailto:${voucherEmail}?subject=${encodeURIComponent("Can you vouch for me on TrustCard?")}&body=${encodeURIComponent(`Hey,\n\nI just set up my TrustCard profile — it's a reputation page that shows verified vouches from people I've worked with.\n\nWould you be willing to give me a quick vouch? It only takes 2 minutes:\n\n${vouchLink}\n\nThanks!`)}`}
            className="flex items-center justify-center gap-2 rounded-xl border border-white/10
                       bg-white/5 px-4 py-3 text-sm text-white/60 hover:text-white hover:border-white/20 transition-colors">
            <span>✉️</span> Email
          </a>
    ...
    return (
      <Shell step={3}>
        ...
        <p className="text-sm text-white/40 mb-6 leading-relaxed">
          ...
          Who is the <span className="text-white/70 font-medium">one person</span> you'd most want to vouch for you?
        </p>
        <div className="grid grid-cols-2 gap-2 mb-5">
          {VOUCH_SUGGESTIONS.map((s, i) => (
            <button key={i} onClick={() => { setSelectedSuggestion(i); setVoucherEmail(""); }}
              className={`flex items-center gap-2 px-3 py-3 rounded-xl border text-sm font-medium
                transition-all duration-200 text-left min-h-[48px]
        ...
        <AnimatePresence>
          {selectedSuggestion !== null && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="mb-2">
              <label className="text-xs text-white/30 uppercase tracking-widest block mb-2">Their email</label>
              <input type="email" className={inputCls}
                placeholder={VOUCH_SUGGESTIONS[selectedSuggestion].placeholder}
                value={voucherEmail} onChange={(e) => setVoucherEmail(e.target.value)} autoFocus />
            </motion.div>
          )}
        </AnimatePresence>
        <p className="text-[11px] text-white/20 leading-relaxed mb-1">
          They'll get a link — no account needed. Takes 2 minutes and immediately appears on your card.
        </p>
        <NavButtons onNext={sendFirstVouch}
          nextLabel={vouchSending ? "Sending…" : "Send vouch request →"}
          canNext={selectedSuggestion !== null && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(voucherEmail) && !vouchSending}
