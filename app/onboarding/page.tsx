...
function OnboardingInner() {
  ...
  async function createProfile() {
    ...
    // Fix: guard against overwriting an existing profile
  ...
  async function sendFirstVouch() {
    ...
    // Fix: use NEXT_PUBLIC_SITE_URL so the link works on any device
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ||
      (typeof window !== "undefined" ? window.location.origin : "");
    setVouchLink(`${siteUrl}/vouch/${data.public_token}`);
  }
  ...
  if (step === 2) return (
    <Shell step={2}>
      ...
      <div className="space-y-3">
        <input className={inputCls} placeholder="School or university" value={school}
          onChange={(e) => setSchool(e.target.value)} />
        <input className={inputCls} placeholder="Major or field" value={major}
        ...
        <div className="grid grid-cols-2 gap-2 mb-6">
          <a
            ...
            className="flex items-center justify-center gap-2 rounded-xl border border-white/10
                       bg-white/5 px-4 py-3 text-sm text-white/60
      ...
      <Shell step={3}>
        ...
        <div className="grid grid-cols-2 gap-2 mb-5">
          {VOUCH_SUGGESTIONS.map((s, i) => (
            <button key={i}
              onClick={() => { setSelectedSuggestion(i); setVoucherEmail(""); }}
              className={`flex items-center gap-2 px-3 py-3 rounded-xl border text-sm
                font-medium transition-all duration-200 text-left min-h-[48px]
        ...
        <p className="text-[11px] text-white/20 leading-relaxed mb-1">
          They'll get a link — no account needed. Takes 2 minutes and
          immediately appears on your card.
        </p>
        <NavButtons
